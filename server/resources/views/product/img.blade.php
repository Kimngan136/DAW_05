                                ...
                                @foreach($images as $index => $img)
                                    <div class="mr-3 mb-3 position-relative">
                                        ...
                                        <div class="overlay position-absolute top-0 start-0 d-none">
                                            <a href="#" class="btn btn-sm btn-warning mt-1 btn-edit"
                                                data-id="{{ $img->id }}">Sửa</a>
                                            @if($index > 0) <!-- Show delete button for images other than the first one -->
                                                <form action="{{ route('product.delete-image', $img->id) }}" method="POST"
                                                    class="d-inline">
                                                    @csrf
                                                    @method('DELETE')
                                        ...
                                        <div class="edit-form d-none" id="edit-form-{{ $img->id }}">
                                            <form action="{{route('product.update-color', $img->id)}}" method="POST"
...
<form method="POST" action="{{ route('product.hd-update-images', $id) }}" enctype="multipart/form-data">
    ...
    @foreach($uniqueColors as $productDetail)
        <div class="form-group">
            ...
            <input id="img_{{ $productDetail->id }}" title="{{$productDetail->color->name}}" type="file"
                name="img[{{ $productDetail->color->id }}][]" multiple accept="image/*" class="form-control-file">
        </div>
    @endforeach
